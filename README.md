...
├── README.md                  # [본 파일] 하네스 아키텍처 및 메인 명세서
├── AGENTS.md                  # 에이전트 최상위 코딩 수칙 및 동작 지침 가이드 (7장 시스템 규약)
├── .agent.config.json         # 프로젝트별 빌드/테스트 CLI 실행 명령어 커스텀 설정
├── .agent/                    # 에이전트 하네스 코어
│   ├── bin/                   
│   │   ├── init.sh            # 하네스 초기화 및 Git Hook 동기화 스크립트
│   │   └── run-test.sh        # 프로젝트별 린트 및 기동 테스트 실행 래퍼
│   ├── prompts/               # 서브 에이전트용 특화 지침 프롬프트 목록
│   │   ├── qa_engineer.md     # 기동 테스트 실패 전문 디버깅 에이전트
│   │   ├── security_auditor.md# 린트 및 보안 수칙 감사 에이전트
...
│   │   └── architecture.rule.md# 계층 구조 침범 방지 및 도메인 의존성 역전 아키텍처 규칙
│   └── hooks/
...
└── docs/                      # 에이전트 전용 로컬 지식 저장소
    ├── adr/
    │   └── README.md          # [에이전트 이정표] 과거 결정 기록(ADR) 작성 가이드 및 목적 명시
    └── failures/
        └── README.md          # [에이전트 오작동 차단] 실패 기록(Failures) 작성 가이드 및 목적 명시
```
...
*(최초 1회 수동으로 생성해 둘 수 있으나, 아래 3단계 지능형 설정을 통해 자동으로 빌드 가능합니다)*
```json
{
  "project_type": "nodejs",
...
[사용자 요청] -> [메인 에이전트(AGENTS.md) 작업 진행] 
          ...
          [.agent/bin/run-test.sh 실행]
         ┌───────────┴───────────┐
         ▼ (성공)                ▼ (실패)
   [커밋 최종 승인]        [.agent/hooks/on-test-fail.sh 트리거]
                                 │
                                 ▼ (자동 피드백 루프)
...
| :--- | :--- | :--- |
| **`/init`** | `./.agent/bin/init.sh` | 로컬 임시 디렉토리 및 Git 로컬 pre-commit 훅 자동 복사 및 동기화 |
| **`/test`** | `./.agent/bin/run-test.sh` | `.agent.config.json` 설정을 읽어 린트 및 기능 테스트를 즉시 일괄 수행 및 보고 |
| **`/clean`** | `./.agent/hooks/post-task.sh` | 빌드 시 생성된 `.tmp_artifacts/` 하위의 가비지 캐시 및 임시 로그 아티팩트 청소 |
...
* **코딩 가이드 확장**:
  * `.agent/rules/guidelines.rule.md`를 편집하여 프로젝트 고유의 컨벤션(예: "모든 함수 상단에는 한글로 목적 주석 작성")을 에이전트에게 강력하게 강제 적용할 수 있습니다.
